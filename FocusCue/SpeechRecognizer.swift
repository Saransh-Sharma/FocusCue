...
class SpeechRecognizer {
    ...
    func start(with text: String) {
        ...
        case .denied, .restricted:
            error = "Microphone access denied. Open System Settings → Privacy & Security → Microphone to allow FocusCue."
            ...
            AVCaptureDevice.requestAccess(for: .audio) { [weak self] granted in
                DispatchQueue.main.async {
                    if granted {
                        ...
                        self?.error = "Microphone access denied. Open System Settings → Privacy & Security → Microphone to allow FocusCue."
                    }
                }
            }
            return
    ...
    private func requestSpeechAuthAndBegin() {
        SFSpeechRecognizer.requestAuthorization { [weak self] status in
            DispatchQueue.main.async {
                ...
                default:
                    self?.error = "Speech recognition not authorized. Open System Settings → Privacy & Security → Speech Recognition to allow FocusCue."
